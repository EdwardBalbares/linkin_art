...
if (isset($_GET['fix_admin_password'])) {
    include_once('config/dbconnect.php');
    $newHash = password_hash('admin123', PASSWORD_BCRYPT);
    $sql = "UPDATE users SET password = '" . $newHash . "' WHERE email = 'admin@gmail.com' AND role = 'admin'";
    ...
    if ($stmt = mysqli_prepare($conn, $query)) {
        ...
        if ($user = mysqli_fetch_assoc($result)) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                // Ensure the admin_logins table exists before inserting
                $conn->query("CREATE TABLE IF NOT EXISTS admin_logins (
                ...
                $stmt_log = $conn->prepare("INSERT INTO admin_logins (admin_id, login_time) VALUES (?, ?)");
...
<div class="login-container position-relative">
    ...
    <div class="text-center mt-3">
        ...
        <a href="create_account.php?role=admin" class="create-account-link"><i class="fas fa-user"></i> Create admin Account</a>
    </div>
    <style>
    .icon-back-btn {
        color: #2ecc71;
...
<script>
// Prevent logged-in admin users from accessing admin login page via back button
if (window.history && window.history.pushState) {
    ...
    window.addEventListener('popstate', function() {
        // If trying to go back and user is logged in as admin, redirect to admin panel
        <?php if (isset($_SESSION['user_id']) && isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
            window.location.href = 'index.php';
        <?php else: ?>
            // If not logged in as admin, stay on admin login page
            window.history.pushState(null, null, window.location.href);
        <?php endif; ?>
    });
}

// Prevent access to admin login page via back button when already logged in as admin
